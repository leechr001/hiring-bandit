...
\newcommand{\TimeToSwitch}{\textsc{TimeToSwitch}}
...
%
In this paper, we study a sequential workforce management problem in a contingent labor setting with worker availability uncertainty. A firm seeks to maximize cumulative productivity by maintaining an active team of fixed size while learning worker quality over time. We incorporate two operational frictions that are often abstracted away: replacing workers is costly and takes time to complete. Such delays arise naturally because newly hired workers may require onboarding, training, or administrative processing before becoming productive. As a result, when the firm initiates several replacements, the workforce may temporarily pass through mixed intermediate teams that were not intended as final staffing decisions and may perform poorly. We formulate this problem as a stochastic multi-play bandit with delayed and costly replacements. {%\color{blue} 
We develop a learning-based hiring policy, \textsc{Optimistic-Hire}, that makes hiring decisions sequentially via learning cycles. In each cycle the policy determines when to initiate workforce changes, which workers to replace and hire, and how to pair outgoing and incoming workers in order to optimize the performance of these intermediate teams. We show that the regret of the proposed policy matches the lower bound in its leading-order dependence on the time horizon, up to logarithmic and lower-order friction terms. Numerical experiments show that \textsc{Optimistic-Hire} performs strongly relative to the benchmark and heuristic adaptations we study in the calibrated settings.}
...
Temporary and contingent work, including gig and freelance work, has become a defining feature of modern labor markets \citep{wood2019good, wu2024gig}. Firms across industries increasingly rely on these arrangements to manage volatile demand and maintain operational flexibility, yet conventional screening tools often provide only limited information about who will perform well on the job. In many operational settings, the most informative signal of worker quality is therefore firm-specific observed performance rather than pre-hire credentials alone.

At the same time, operational data systems now make those performance signals easier to observe at high frequency. For example, Amazon reportedly tracks, down to the second, how long it takes a warehouse associate to retrieve a product \citep{Vincent2015}. In platform and staffing settings, a firm may repeatedly draw from a finite menu of pre-screened contingent workers, observe throughput or quality during each engagement, and then decide whom to retain and whom to replace, often while paying a placement fee for each new booking.

Those replacement decisions are not instantaneous. A booked worker may not become productive until several periods later because of availability constraints, onboarding, or administrative processing. As a result, the realized workforce can pass through transitional intermediate teams that are neither the current team nor the intended final team. This distinction between the workforce the firm requests and the workforce it actually operates with is central in contingent hiring and does not arise in standard \(m\)-play bandit models with instantaneous actions.

Building on these observations, in this paper we study a sequential hiring problem in which a firm maintains an active team of fixed size while learning worker quality over time (e.g., a delivery service operating a fleet of trucks, or a call center with a fixed number of terminals). The formulation is intended for finite planning settings with a moderate candidate pool or with prior data on most workers. It is less appropriate for effectively unbounded worker pools, richly contextual screening environments, rolling or unknown horizons, or settings with strategic worker-side behavior. We return to these scope boundaries in the main text below because they matter for the managerial interpretation of the results.
%in this paper w
...
In this paper, we formulate the sequential hiring of contingent workers as a stochastic multi-play bandit problem with switching costs and random action delays. 
The \emph{switching costs} capture financial frictions such as platform placement fees, administrative onboarding costs, and paid training, while the random action delays capture worker-supply uncertainty, recruiting procedures, scheduling delays, and onboarding.
...
We formulate hiring as a bandit problem with switching costs and delayed actions, in which replacement decisions take effect only after a random amount of time. Because each replacement incurs a switching cost, it is crucial to balance 
%\textsc{Optimistic-Hire} (\textsc{OH}), that balances 
information acquisition against switching costs and the performance degradation induced by delayed actions. To the best of our knowledge, this multi-play bandit problem with switching costs and delayed arm availability has not been studied in the literature. Thus, the model, theoretical results, and the regret analysis in this paper are new and of independent interest.

% (Internal author comment removed.)
%We design a learning-based hiring policy, \textsc{Optimistic-Hire} (\textsc{OH}), that balances information acquisition against switching costs and the performance degradation induced by delayed actions, thereby improving workforce composition over time while controlling the pace of turnover.


{\bf Methodological contributions.}
We develop a learning-based hiring policy, \textsc{Optimistic-Hire}, that makes hiring decisions sequentially through learning cycles. We show that the regret of the proposed policy matches the lower bound in its leading-order dependence on the time horizon, up to logarithmic and lower-order friction terms. 
The new features in our model create
...
challenges absent from the analysis of standard bandit models. 
...
Third, delayed replacements introduce a new implementation question absent from standard bandit models: once the policy has chosen which workers should leave and enter, how should those workers be paired during the transition? Different pairings generate different \emph{intermediate workforce} configurations, a phenomenon unique to our model (see Remark~\ref{ex:intermediate-set}). As a complementary contribution, we study a simple and interpretable Rank-Matching rule that pairs outgoing and incoming workers according to their lower-confidence rankings. Under worst-case delays it maximizes a conservative surrogate of transition performance, whereas under i.i.d.\ finite-support delays all bijections have the same expected surrogate value and rank matching instead minimizes transition variability. These pairing results are auxiliary to the main regret analysis: the leading-order regret guarantees are driven by the switching and selection components, while the pairing analysis clarifies how the implementation rule affects transition quality.
...
The known horizon enters the policy only through the screening term in \ChooseTarget and through the offline choice of \(\gamma\); the adaptive switching rule itself is horizon-free. At a high level, the regret analysis combines three ideas. First, we convert the adaptive switching rule into a counting argument by lower bounding, pathwise, the growth of the least-observed worker in the current target workforce. Second, we decompose sampling regret into a standard identification term and a new delay-induced term that telescopes across learning cycles. Third, we separate the pairing problem from the main regret proof and analyze the rank-matching rule under two stylized delay models to explain how pairing affects transition losses without claiming that it drives the main regret rate.

The rest of the paper is organized as follows. The next section reviews the related literature. Section~\ref{formulation} presents the problem formulation, and Sections~\ref{sec:alg-desc} and \ref{theoretical} develop the learning-based \textsc{Optimistic-Hire} policy and establish its theoretical performance guarantees. Section~\ref{numerical} reports the core benchmark, delay, cost, and staffing-ratio experiments. The appendices contain the detailed proofs, the pairing analysis, the dynamic-programming details for \ChooseTarget, and additional sensitivity studies on \(\gamma\), gap structure, threshold robustness, benchmark-adaptation diagnostics, and policy-component ablations. Section~\ref{sec:conclusion} concludes with directions for future research.

\section{Literature Review
%Related Work
}
The present work builds on two bodies of literature: dynamic workforce management and multi-armed bandit learning. We therefore review each separately below, focusing only on the most relevant works. %{\color{red} This literature review should be shortened, currently it is 4.5 pages. I have shortened it somehow, but need to shorten further.}

\vspace{-.1in}

\subsection{Learning-Based Hiring and Workforce Management}

Hiring and workforce allocation are core operational activities that have inspired extensive theoretical research. A classical starting point is the \emph{secretary problem} \citep{dynkin1963optimal, ferguson1989who}, which models sequential hiring under uncertainty, and its extensions \citep{kleinberg2005multiple, disser2020hiring, kesselheim2024hiring}. While these models capture fundamental aspects of information arrival and timing, they assume that candidate quality is immediately observable and static, and that decisions are permanent once made.
...
When the firm wishes to change work assignments by replacing worker \(i\) with worker \(j\), the change does not take effect immediately. Instead, the action enters a delay process, capturing frictions such as onboarding steps, platform-mediated communication, scheduling constraints, or the worker's own availability, before worker \(j\) becomes active. We model this friction by associating each replacement \((i,j)\) initiated in period \(t\) with an independent, nonnegative-integer-valued delay \(\omega_{i,j}^{(t)}\). We assume that \(\bE{\omega_{i,j}^{(t)}} \le \omega_{\max} < \infty\) for all \(i,j\) and \(t\), where \(\omega_{\max}\) is known to the firm. Since decisions are made at the beginning of each period, the event \(\omega_{i,j}^{(t)}=0\) corresponds to the case in which the replacement is realized in the same period before rewards are generated, whereas \(\omega_{i,j}^{(t)}=1\) means the replacement first affects the next period's rewards. The analysis can also be extended to adaptive or adversarial delay processes when the delays have finite support.
...
\[
    \mathcal{E}_t := \{(i,j) \;:\; C_{i,j}^{(n)} = t \text{ for some } n \le t\}
\]
...
{\bf Switching costs.} Each initiated replacement incurs a fixed switching cost \(c > 0\). This cost reflects administrative tasks, coordination, brief training, and possible platform fees associated with onboarding a new worker. For convenience, we assume that the cost is incurred when the replacement is initiated. The total switching cost in period \(t\) is therefore \(c \cdot |R_t|\).

\vspace{-.1in}

\subsection{Active Workforce and Performance Measure}
The implementation distinguishes the workforce the policy sees at the start of period \(t\) from the workforce that ultimately generates period-\(t\) rewards. The policy acts on the carried-over roster from the end of period \(t-1\), then the environment samples delays for accepted requests and realizes all replacements in \(\mathcal{E}_t\), and only afterward are rewards drawn. For ease of notation, we continue to use \(A_t\) for the reward-generating active workforce in period \(t\), while the algorithm description below makes the start-of-period decision timing explicit. With this convention, the active roster evolves only through the completion of initiated replacements. For any replacement \((i,j)\) with completion time \(C_{i,j}^{(\ell)} = t\), worker \(i\) leaves the roster and worker \(j\) enters it in period \(t\). The workforce update is therefore
\[
    A_t
...
\end{eqnarray}
The expectation is taken over the policy's internal randomness and the workers’ performance realizations. A policy with smaller regret is therefore closer, in expectation, to the performance of an oracle that knows all worker qualities in advance. Note that an oracle policy incurs no switching cost or action delays since it would employ only the workforce \(A^*\).

Table~\ref{tab:assumption-map} summarizes the assumptions used by the main theoretical, pairing, and numerical results. The main regret analysis studies a known-horizon policy under stochastic rewards and delayed replacements with bounded expected duration. The pairing results use different delay models because they analyze transition quality rather than regret: Proposition~\ref{lem:wc-rank-matching} uses a worst-case bounded-delay criterion, whereas Propositions~\ref{lem:iid-rank-matching-E} and \ref{lem:iid-rank-matching-Var} assume i.i.d.\ finite-support delays. The numerical study fixes a finite horizon and uses i.i.d.\ uniform delays. Knowledge of the horizon \(T\) enters only through the screening term \(c/(T-t)\) in \ChooseTarget and through the offline choice of \(\gamma\); the adaptive switching rule \TimeToSwitch does not itself use \(T\). If \(T\) were unknown, one could replace the screening rule with an anytime alternative or use a doubling scheme, but that extension is outside the scope of this paper. More broadly, the model is best suited to settings with a moderate effective candidate pool, stable task requirements, and exogenous delays. Large pools with rich worker covariates suggest contextual or two-stage screening models; rolling-horizon environments suggest anytime variants; and nonstationary productivity or endogenous worker participation would require different learning models altogether.

\begin{table}[t]
    \centering
    \small
    \setlength{\tabcolsep}{4pt}
    \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}} p{0.33\linewidth} p{0.27\linewidth} p{0.32\linewidth}}
    \toprule
    Result & Delay assumptions & Other assumptions \\
    \midrule
    Proposition~\ref{888}, Theorems~\ref{thm:main} and \ref{thm:delta-main} & Nonnegative-integer-valued delays with \(\mathbb E[\omega_{i,j}^{(t)}]\le \omega_{\max}\); the proof works for the stochastic model stated here and extends to finite-support adaptive/adversarial delays & Stochastic bounded rewards; \(m\) identical positions; known horizon \(T\) for \ChooseTarget and tuning \\
    Proposition~\ref{lem:wc-rank-matching} & Arbitrary delays in \(\{0,\dots,\omega_{\max}\}\) & Pairing analysis only; no stochastic delay model required \\
    Propositions~\ref{lem:iid-rank-matching-E} and \ref{lem:iid-rank-matching-Var} & i.i.d.\ delays on \(\{0,\dots,\omega_{\max}\}\) & Pairing analysis only; used to study expected transition reward and variance \\
    Numerical experiments & i.i.d.\ uniform delays on \(\{\texttt{delay\_lower},\dots,\omega_{\max}\}\) with \(\texttt{delay\_lower}=0\) in the no-delay case & Fixed finite horizon; calibrated fulfillment-center setting with \(k=150\) and \(m\in\{50,100\}\) \\
    \bottomrule
    \end{tabular*}
    \caption{Assumption map for the main results.}
    \label{tab:assumption-map}
\end{table}


\section{Optimistic-Hire: A Learning-Based Policy}\label{sec:alg-desc}
We develop a learning-based hiring policy for the delayed replacement setting, where staffing changes take effect after a delay and each replacement incurs a cost. These features introduce a three-way design problem.

First, the policy must determine \emph{when} to initiate replacements. The switching schedule controls a fundamental tradeoff: frequent updates accelerate learning but incur higher cumulative switching costs, while infrequent updates reduce costs but slow adaptation. Second, the policy must decide \emph{which} workers to employ. This is a combinatorial exploration–exploitation problem over subsets of size \(m\), where the firm balances learning about uncertain workers against exploiting those currently believed to be productive. Third, the policy must determine \emph{how} to implement replacements. Because replacements are delayed and realized over time, the pairing between outgoing and incoming workers affects the sequence of intermediate workforces encountered during the transition. We encode these decisions through three components:
\[\textsc{TimeToSwitch}(\cdot),\quad \textsc{ChooseTarget}(\cdot),\quad \textsc{ConstructBijection}(\cdot),\]
corresponding to the switching rule, selection rule, and pairing rule, respectively. All three components rely on common performance estimates and confidence bounds, which we introduce next.
...
\begin{algorithm}
    ...
    \For{time \(t = 1,2,\dots,T\)}
        \State Observe carried-over workforce state and current statistics \((A_t,\mathbf{n}_{t},\boldsymbol{\mu}_{t})\)
        \If{\textsc{TimeToSwitch}(\(A_t, U_{\ell},\mathbf{n}_t, \gamma\))}
            \State Update switch counter: \(\ell \gets \ell+1\)
            \State Pick target: \(U_\ell \gets \textsc{ChooseTarget}(A_t, \mathbf{n}_t, \boldsymbol{\mu}_t)\)
            \State Construct replacement pairs: \(R_t \gets \textsc{ConstructBijection}(A_t,U_\ell, \mathbf{n}_t, \boldsymbol{\mu}_t)\)
        ...
        \State Carry the realized workforce into the next period: \( A_{t+1}\gets\big( A_{t} \setminus \{  i : (i,j) \in \mathcal{E}_t  \} \big) \cup \{  j : (i,j) \in \mathcal{E}_t  \} \)
    \EndFor
    \end{algorithmic}
\end{algorithm}

Algorithm~\ref{alg:Optimistic-Hire} reduces the policy design problem to three interacting components. The switching rule determines when to switch target workforces and incur replacement costs, the selection rule identifies the target workforce given current information, and the pairing rule translates this target into a concrete set of replacements. The switching and selection components are the parts used in the main regret proof. The pairing rule is included because delayed replacements create intermediate teams, but its analysis is auxiliary and is used to explain transition quality rather than the leading-order regret rate. In the implementation, a newly requested replacement with \(\omega=0\) is realized in the same call to the environment step, so the reward-generating workforce in period \(t\) can differ from the roster on which the policy conditions at the start of period \(t\). We next define each component in turn, beginning with the switching rule, followed by the pairing rule, and finally the selection rule.

\begin{remark}[One learning cycle in a small example]
    Consider \(m=2\) and suppose the active workforce at time \(t\) is \(A_t=\{1,4\}\). If worker \(4\) is the least-observed worker in the current target workforce, then \TimeToSwitch waits until worker \(4\) has accumulated the additional observations prescribed by \(N(\ell)\). Once that threshold is reached, \ChooseTarget may replace worker \(4\) with an uncertain outside worker \(3\) only if the optimistic gain \(\delta_{4,3}(t)\) is large enough to justify the switching cost over the remaining horizon. If this replacement is requested and completes two periods later, then the active workforce remains \(\{1,4\}\) during the delay, transitions to \(\{1,3\}\) once the request completes, and the algorithm then holds \(\{1,3\}\) long enough to collect the next block of observations. This illustrates the roles of the switching rule, the horizon-aware selection rule, and the distinction between the active and target workforces.
\end{remark}

\vspace{-.1in}
\subsection{Switching Rule}

The switching rule \TimeToSwitch(\(\cdot\)) governs when the policy is allowed to initiate a new round of replacements. Its purpose is to balance two competing objectives: switching too frequently accelerates adaptation but incurs excessive replacement costs, while switching too infrequently reduces costs but slows learning. A natural way to manage this tradeoff is to require the policy to continue collecting data from the current workforce until a prescribed observation threshold is reached, and to permit further replacements only after that point. If these thresholds are chosen appropriately, the policy can remain adaptive while limiting the frequency of workforce updates.

Because the quality of the current workforce is only as well understood as its least-observed worker, it is natural to define the switching threshold in terms of the minimum number of observations among workers currently targeted for employment. Motivated by this principle, we regulate the pace of workforce updates through an observation-based threshold \(N(\ell)\), which specifies how much additional information must be collected before the next switch is allowed. Specifically, we set
...
The subroutine \TimeToSwitch(\(\cdot\)) is described in Algorithm \ref{alg:TimeToSwitch}. It implements this rule by waiting until the least-observed worker in that workforce has accumulated at least \(N(\ell)\) additional observations since the last switch time \(d_\ell\).

\begin{algorithm}
    \caption{TimeToSwitch}
    \label{alg:TimeToSwitch}
    \begin{algorithmic}[1]
        ...
        \Require Switching parameter \(\gamma>0\)
        \Statex \textit{//===========================}
        \State \textbf{internal state:} Most recent switch time \(d_\ell\)
        \State \textbf{internal state:} Counts at last switch time \(\mathbf{n}_{d_{\ell}}\)
        ...
        \If{\(T_{i^{-}}(t)\geq T_{i^{-}}(d_{\ell}) + N(\ell)\) and \(A_t=U_\ell\)}
            ...
            \State \Return True \Comment{Time to switch!}
        \Else
...
We make three remarks. First, Algorithm~\ref{alg:TimeToSwitch} recomputes the least-sampled worker and the threshold each time the subroutine is called. This can be implemented more efficiently, but we present it this way for clarity. Second, switching is permitted only when the current target workforce \(U_\ell\) is fully active. This ensures that replacements satisfy the coherence constraints \eqref{eq:coherence-1} and \eqref{eq:coherence-2}. Third, the threshold \(N(\ell)\) is defined in terms of accumulated employment periods rather than calendar time. Because replacements are completed asynchronously, the number of calendar periods required to collect these observations is not known in advance. The count-based rule guarantees that sufficient data are collected even when delays postpone the onset of learning for newly hired workers. This differs from the fixed calendar-time schedules in \cite{agrawal1990switching, gao2019batched, perchet2016batched}, where switching opportunities are fixed in advance in calendar time. As shown in Section~\ref{numerical}, this adaptivity is a key source of robustness in delayed environments.
...
\begin{figure}[H]
    ...
    \node[draw, dashed] at (4,1.4) {\textsc{TimeToSwitch}(\(\cdot\)) = \textsc{True}};
    ...
    \node[draw, dashed] at (10,1.4) {\textsc{TimeToSwitch}(\(\cdot\)) = \textsc{True}};
...
This switching rule also induces a useful cyclic structure on the timeline of the algorithm, shown in Figure~\ref{fig:intro-timeline}. Each cycle begins when \TimeToSwitch(\(\cdot\)) returns True and a new target workforce \(U_\ell\) is selected. The workforce then passes through a transition phase, highlighted in red, during which the active workforce differs from \(U_\ell\) because the requested replacements have not yet fully completed. During this phase, the firm may operate with intermediate workforces. Once the replacements are realized, the algorithm enters a phase, highlighted in blue, in which the active workforce equals \(U_\ell\). The policy remains in this phase until the observation threshold in equation \eqref{eq:block-threshold} is met, at which point a new target workforce \(U_{\ell+1}\) is chosen and the cycle repeats. It is worth noting that in some cycles either phase may be empty. 

In each cycle, there is a phase during which the firm may need to operate with intermediate teams. The composition of these teams depends on how incoming and outgoing workers are paired. This motivates a principled study of the pairing rule, which is the focus of the next subsection.

\vspace{-.1in}
\subsection{Pairing Rule}
Next we introduce the pairing rule \ConstructBijection(\(\cdot\)). Given a target workforce \(U_{\ell}\) and an active workforce \(A_t\), the pairing rule aims to minimize the regret incurred by the intermediate workforce containing some workers from \(U_{\ell}\) and some workers from \(A_t\). The key idea behind this rule is a \emph{rank-matching bijection}, which we introduce next.
...
\end{equation}
That is, workers to be removed and workers to be added are paired according to their relative lower-confidence rankings. The subroutine \ConstructBijection(\(\cdot\)) constructs this pairing between the target and active workforce:

\begin{algorithm}
    \caption{ConstructBijection}
    \label{alg:ConstructBijection}
    \begin{algorithmic}[1]
        \Require Current active workforce \(A_t\)
    ...
    \end{algorithmic}
\end{algorithm}
Because replacements are completed after heterogeneous delays, the pairing rule affects the sequence of intermediate workforces encountered during the transition. The rank-matching bijection is particularly appealing in this regard: it satisfies several min--max properties with respect to a surrogate measure of transition regret. These results are formalized in Appendix \ref{sec:bijection_analysis}. They should be interpreted as complementary implementation guidance rather than as ingredients in Theorem~\ref{thm:main}: the regret analysis does not rely on the pairing propositions, and under i.i.d.\ delays the main benefit of rank matching is variance control rather than an improved leading-order mean regret term.

\subsection{Selection Rule}
We now turn to the final component of the algorithm: the selection rule \ChooseTarget(\(\cdot\)), which determines the target workforce \(U_\ell\). A natural starting point is an optimism-based rule that selects the subset with the largest aggregate upper confidence bound. In our setting, however, switching costs and a known planning horizon \(T\) require a refinement: near the end of the horizon, the remaining time may be too short for a replacement to recover its cost unless the potential improvement is sufficiently large. The subroutine \ChooseTarget(\(\cdot\)) incorporates this horizon-aware switching criterion into an otherwise optimistic selection rule.
...
\begin{algorithm}
    ...
    \end{algorithmic}
\end{algorithm}
The objective in Algorithm~\ref{alg:ChooseTarget} is optimistic, favoring target workforces with large upper-confidence values. The constraint tempers this optimism by requiring that the new workforce can plausibly recover its switching cost over the remaining horizon.

\begin{remark}[Computation of \textsc{ChooseTarget}]
    The constrained selection step is not solved by brute force. For a fixed number \(r\) of replacements, the entering side can be chosen greedily as the \(r\) inactive workers with the largest UCB values; the remaining task is to choose which \(r\) currently active workers to remove. This reduces \ChooseTarget to a cardinality-constrained knapsack problem on the active workforce. In Appendix \ref{sec:comp_of_selection_rule}, we develop an exact frontier-based dynamic program and a \((1-\epsilon)\)-approximation variant based on the monotonicity property proved in Appendix \ref{sec:sampling_proof}. The method is output-sensitive: its runtime is polynomial in \(m\) and in the size of the maintained Pareto frontier, rather than in \(\binom{m}{r}\). We return to the practical implications of this observation in Section~\ref{numerical}.
\end{remark}

\section{Theoretical Performance Guarantee}
\label{theoretical}
Having established the operational rationale for pacing staffing adjustments in the presence of delays and switching costs, we now turn to the performance implications of this approach. The following results show that adapting to delayed actions need not come at the expense of learning efficiency. Before establishing our main result, we provide a lower bound for combinatorial semi-bandits with delayed actions and costly replacements.

\begin{proposition} [Minimax lower bound]
\label{888}
For any \(m\) and \(k\), and any horizon \(T > 0\), there exists a combinatorial semi-bandit problem with delayed and costly replacements such that the regret of any algorithm is
\begin{equation*}
    \Regret(T) \geq \min \left\{\Omega\left(\sqrt{mkT}\right), \Omega\left(mT\right) \right\}.
\end{equation*}
\end{proposition}

The proof appears in Appendix \ref{sec:lower_bounds}. We now present our main theoretical result. When equipped with a suitable choice of the parameter \(\gamma\), our hiring policy achieves the same dominant \(\sqrt{T}\)-dependence as the lower bound in Proposition \ref{888}, up to logarithmic factors and lower-order friction terms. Delays and switching costs enter through additive terms that can be controlled via \(\gamma\), which governs how aggressively the policy is permitted to adjust the workforce. 

\begin{theorem}[Instance-independent regret bound]\label{thm:main}
    There exist universal constants \(C_1, C_2, C_3>0\) independent of \((k,m,T,c,\omega_{\max})\) and \(\gamma\) such that for all \(k\ge 2\),
    \begin{eqnarray}
    \label{099}
        \Regret(T) &\le&  C_1 \sqrt{m(k-m)T\log T} 
         + C_2(k-m)\sqrt{\gamma \polylog(T)} \\
        &&\qquad\qquad\qquad\qquad + C_3 k\left(\gamma + (\omega_{\max}+c)m\right)
        + (\omega_{\max}+c)m\sqrt{\frac{kT}{\gamma}}
        \nonumber
    \end{eqnarray}
    Additionally, when \(\gamma = (\omega_{\max}+c)^2m\), there are different constants \(C_4, C_5\), independent of the problem parameters, such that
  \begin{eqnarray}
      \Regret(T) \leq C_4 \sqrt{mkT\log T} 
        +\; C_5km(c+\omega_{\max})^2\sqrt{ \polylog(T)}.
        \label{088}
        \end{eqnarray}
    The exact constants are given in Appendix \ref{sec:thm-all-constants}.
\end{theorem}

{\bf Explanation of regret.} This simplified bound has four distinct contributions that can be interpreted as follows. The term \(C_1 \sqrt{mkT\log T}\) has the same form as in frictionless models. It represents the intrinsic statistical difficulty of learning which workers perform well, independent of the operational constraints. The lower-order term, \(C_2\,(k-m)\sqrt{\gamma \polylog(T)}\), captures the additional learning cost induced by limiting the rate of switches. Increasing the parameter \(\gamma\) lengthens the periods over which the workforce configuration is held fixed, reducing the frequency of updates but slowing the rate at which suboptimal workers can be identified and replaced. As a result, the learning regret grows with \(\sqrt{\gamma}\) and scales linearly with the number of suboptimal workers, \(k-m\). The remaining dependence on the horizon \(T\) enters only through polylogarithmic factors. The third term \(C_3\,k\left(\gamma + (\omega_{\max}+c)m\right)\), 
...
% In the presence of execution delays and switching costs, acquiring even a single observation from a worker may incur a delay of up to \(\omega_{\max}\) periods and a switching cost of \(c\). Since the algorithm must obtain at least one observation from each of the \(k\) workers, this requirement alone induces an unavoidable cost proportional to \(k(\omega_{\max}+c)\). For comparison, related algorithms in settings with instantaneous actions and no switching costs, such as \cite{kveton2014matroid}, incur a corresponding start-up cost of order \(km\). 
The remaining term, \((\omega_{\max}+c)\,m\sqrt{{kT}/{\gamma}}\), isolates the penalty of delayed actions and switching costs over time. It scales like \(\sqrt{T}\) and decreases in \(\gamma\). More conservative updates (larger \(\gamma\)) reduce the cumulative impact of frictions by limiting how often the workforce is changed. In contrast, smaller \(\gamma\) makes the policy more agile but amplifies this friction-driven component. 

The second part of Theorem \ref{thm:main}, which chooses \(\gamma = (\omega_{\max}+c)^2 m\), shows that this trade-off can be resolved in favor of learning. Once \(\gamma\) is chosen large enough relative to the magnitude of the frictions, the \((\omega_{\max}+c)m\sqrt{kT/\gamma}\) term becomes \(\tilde{O}(\sqrt{kmT}\,)\), the same order as the frictionless lower bound, and the leading \(\sqrt{T}\) term of the bound becomes independent of \(c\) and \(\omega_{\max}\).

{\bf High switching cost.} In the bound in Theorem \ref{thm:main}, the switching cost $c$ is a constant and independent of $T$. In situations where switching cost is relatively high, we can model $c$ as 
a function of $T$, e.g., $c=\Theta(\log T)$ or $c=\Theta(T^{1/4})$. From (\ref{088}), it is seen that as long as the switching cost grows at rate  
$$c\;\le \; O\big((T/(mk))^{1/4}-\omega_{\max})^+\big),$$ 
...
\begin{proposition}[Instance-dependent regret lower bound]\label{prop:asymptoptic-lower}
    Fix any \(\Delta \in (0,1/2)\), and consider a delayed replacement bandit instance with a Bernoulli reward process with means
    \[\mu_i =\begin{cases}
        1/2, & i = 1,\dots,m,\\
...
\begin{theorem}[Instance-dependent regret bound]\label{thm:delta-main}
    Suppose that there are \(m\) unique best arms (i.e., \(\mu_1\geq \cdots \geq \mu_m >\mu_{m+1}\geq \mu_k\)). Define \(\Delta_{i,j}:=\mu_i-\mu_j\) to be the optimality gap. Then there exist constants \(C_1,C_2,C_3\), and \(C_4\) such that when \(T>k\),
    \begin{align*}
        \Regret(T)&\leq C_1\sum_{j>m}\frac{1}{\Delta_{m,j}} \left( \log T+(\omega_{\max}+c)m\sqrt{\frac{k\log T}{\gamma }}\right)\\
        ...
        &\quad +~C_4(\omega_{\max}+c)mk
    \end{align*}
    Additionally, when \(\gamma=(\omega_{\max}+c)m\), there are different constants \(C_5, C_6\), independent of the problem parameters, such that
    \begin{align*}
        \Regret(T)&\leq C_5\sum_{j>m}\left[\frac{\log T}{\Delta_{m,j}}+\left(\polylog\left(\frac{\Delta_{1,j}}{\Delta_{m,j}}\right)+\frac{1}{\Delta_{m,j}}\right)\sqrt{(\omega_{\max}+c)mk\log T}\right]\ \\
        &\quad +~ C_6\sum_{j>m} \left[c\log\left(\frac{c}{\Delta_{m,j}}\right)+m\Delta_{1,j} \left(\omega_{\max}+c + \frac{1}{T}\right)\right]
    \end{align*}
\end{theorem}
...
In practice, we recommend choosing \(\gamma\) by numerically minimizing the regret upper bound in Theorem \ref{thm:main} over the known parameters \((c, \omega_{\max}, k, m, T)\). These parameters are typically known to the decision maker, so this strategy is implementable in practice. In the numerical experiments of Section \ref{numerical}, we use this choice of parameter.
...
\label{numerical}
This section presents numerical experiments designed to clarify what the proposed policy does well, what aspects of the model drive those gains, and what the experiments do and do not establish. We begin with a calibrated fulfillment-center-inspired benchmark. We then vary the staffing ratio, replacement delays, and switching costs in the main text. Additional appendix experiments examine sensitivity to the tuning parameter \(\gamma\), the optimality-gap structure, the robustness of tuned-threshold heuristics, the tractability of the \ChooseTarget subproblem, and diagnostics for the delayed-action benchmark adaptations.
...
When a new worker is hired, they must complete an onboarding process before becoming productive. We model onboarding as requiring a full 8-hour shift during which the worker generates no output. Moreover, the firm must pay the worker for this time. This assumption is consistent with warehouse training practices, where new hires undergo paid orientation and initial instruction before contributing to throughput. Operationally, this corresponds to a delay of 8 periods \(\omega_{\max}=8\) and an effective switching cost proportional to 8 periods of worker pay, which we model as \(c=8\). Finally, we draw the intrinsic productivity of each worker uniformly between 0.3 and 0.7. 

The \textsc{Optimistic-Hire} parameter \(\gamma\) is chosen to numerically minimize the regret bound with all constants (see Appendix \ref{sec:thm-all-constants}) given the known parameter values \(c\), \(\omega_{\max}\), \(k\), \(m\), and \(T=5\) years. 

Computational tractability is important in this setting because \ChooseTarget is invoked repeatedly over a long horizon. As discussed after Algorithm~\ref{alg:ChooseTarget}, the exact solver is a frontier-based dynamic program rather than an exhaustive search over \(\binom{k}{m}\) workforces. In our local Python implementation of the benchmark calibration \((k=150,m=100,T=43{,}800)\), a full five-year \textsc{Optimistic-Hire} simulation run takes about 8 seconds, the mean time per \ChooseTarget call is about 47 milliseconds, and the largest frontier we observed in a small runtime study was 9 states. Appendix~\ref{sec:comp_of_selection_rule} provides the algorithmic details, and Figure~\ref{fig:frontier-size} shows that in the broader benchmark study the maintained frontier remained very small throughout and never exceeded 12.

{\bf Comparison policies.}
The setting most closely related to ours is the \(m\)-play bandit with switching costs and instantaneous actions. Accordingly, we benchmark against the standard algorithm from this literature, \cite{agrawal1990switching}, which we denote \textsc{AHT}. The second benchmark algorithm we implement is the Optimistic Matroid Maximization algorithm (\textsc{OMM}) from \cite{kveton2014matroid}, which employs an optimism-based selection rule similar to ours and achieves \(O(\sqrt{kmT})\) regret in the absence of delays and switching costs. This choice reflects the canonical regret-optimal approach for stochastic matroid bandits, a class that includes the \(m\)-play setting as a special case.

Because our setting involves delayed actions, both algorithms must be adapted before they can be implemented. First, the original algorithms prescribe workforce selections, whereas in our setting decisions are implemented through replacements. At each decision epoch, we query the algorithm for its desired workforce and then translate that workforce into a set of replacement requests. In the main benchmark, we use a random pairing for these delayed-action adaptations so that the baselines are not strengthened by importing the paper's proposed pairing rule by default. Second, the original algorithms assume instantaneous actions and therefore do not account for the feasibility constraints induced by delays. In particular, proposed replacements may violate the coherence constraints \eqref{eq:coherence-1}--\eqref{eq:coherence-2}, for example by attempting to remove or add a worker who is already involved in a pending replacement. To enforce feasibility, we discard any conflicting replacement requests. More sophisticated delay-aware adaptations may be possible, but they would require substantial modifications to the original policies. For example, for \textsc{AHT}, it is not clear how to preserve its calendar-time-based switching schedule without such modifications, since delayed replacements can prevent the requested workforce changes from being realized on schedule. Accordingly, the numerical results should be interpreted as comparisons against natural delayed-action adaptations of these baselines rather than against the best possible policies for our model class. To make that scope explicit, Appendix~\ref{sec:baseline-diagnostics} reports diagnostic statistics on discarded requests and also compares random pairing with rank-matching for the delayed-action adaptations of \textsc{AHT} and \textsc{OMM}. Appendix~\ref{sec:oh-ablations} complements those external comparisons with internal ablations of \textsc{Optimistic-Hire}: adaptive versus fixed-calendar switching, with versus without the horizon screen in \ChooseTarget, and rank-matching versus random pairing. In Section \ref{numerical:delayed}, we additionally compare \textsc{Optimistic-Hire} with the benchmark policies in the no-delay regime, where no delay-related adaptations are required and any performance differences therefore cannot be attributed to those modifications.

Additionally, we include several heuristics intended to reflect common employment practices. The first benchmark, denoted \textsc{TunedThreshold}(\(\tau\)), is a fixed-threshold screening policy. It retains each worker in the active workforce as long as that worker's empirical mean reward remains at least \(\tau\). Whenever a worker's empirical mean falls below this threshold, the policy replaces that worker with the available alternative having the highest empirical mean, treating unobserved workers as having infinite empirical mean. For each simulation instance, the threshold value \(\tau\) is selected in a preliminary calibration step by evaluating a grid of candidate thresholds with spacing \(0.05\) and choosing the best-performing value. The second is \textsc{SemiAnnualReview}. This policy reviews its active workforce every 6 months and requests replacements to achieve the empirically best-performing workforce. The last is \textsc{WorkTrial}. This policy is based on a blog post from the job-posting website Indeed \citep{indeed_job_trial}. The policy employs each worker for 1 hour, or for as short a tenure as possible (delays may cause this to be longer than 1 hour). It then selects the top \(2m\) workers by empirical mean. The firm employs the first \(m\) for 90 days, then replaces them with the second \(m\) for 90 days. This corresponds to a ``work trial.'' Afterward, the firm permanently retains the \(m\) best-performing workers.
...
At five years, \textsc{Optimistic-Hire} reduces cumulative regret by about \(69\%\) relative to \textsc{AHT}, about \(88\%\) relative to \textsc{OMM}, and about \(48\%\) relative to \textsc{WorkTrial}. It is also the most stable policy across instances. Additionally, \textsc{Optimistic-Hire} reduces the variability of cumulative regret by about \(78\%\) relative to \textsc{AHT}, about \(79\%\) relative to \textsc{WorkTrial}, and about \(82\%\) relative to \textsc{OMM}. Thus, within this benchmark set, \textsc{Optimistic-Hire} not only achieves the best mean performance at the target horizon, but does so substantially more reliably across heterogeneous instances. Appendix \ref{sec:benchmark_tables} provides tables listing cumulative regret and normalized loss at horizons 1 month, 12 months, 3 years, and 5 years.

Appendix~\ref{sec:oh-ablations} helps separate which design choices drive these gains. In the benchmark calibration, replacing the adaptive count-based switching rule by a fixed calendar schedule raises five-year regret from \(11{,}289\) to \(11{,}615\) in a four-run ablation, even though the short-horizon effect is small. By contrast, removing the horizon screen or replacing rank matching with random pairing has little effect in this low-cost i.i.d.\ benchmark. We therefore interpret the main benchmark as evidence primarily for the switching-and-selection architecture of \textsc{Optimistic-Hire}, not as evidence that every component matters equally in every regime.

Figure~\ref{fig:benchmark_50_150} reports results for the lower-utilization regime with \(m=50\), so that a smaller fraction of the worker pool is employed in each period. This provides a robustness check with respect to the staffing ratio. The qualitative pattern is unchanged: \textsc{Optimistic-Hire} again performs best over the medium and long horizons, while remaining competitive early. While \textsc{TunedThreshold} performs best in the short run, it relies on tuning parameters that depend on unknown problem characteristics. Accordingly, it should be viewed as a best-case benchmark rather than as a deployable policy. Appendix~\ref{sec:threshold-robustness} shows that its performance can deteriorate substantially under even mild perturbations of the underlying unknown problem parameters.
...
%     \begin{minipage}[t]{0.48\linewidth}
%         \centering
%         \includegraphics[width=\linewidth]{paper-images/benchmark_cum_regret_15_150.png}
%     \end{minipage}
%     \hfill
%     \begin{minipage}[t]{0.48\linewidth}
%         \centering
%         \includegraphics[width=\linewidth]{paper-images/benchmark_normalized_loss_15_150.png}
%     \end{minipage}

%     \caption{Simulated performance of Optimistic-Hire compared to benchmarks with \(m=15\).}
%     \label{fig:benchmark_15_150}
% \end{figure}
\vspace{-.1in}

\subsection{Effects of Delayed Replacements}\label{numerical:delayed}
In Figure~\ref{fig:sweep_delay}, we examine the effect of delayed replacements by varying the maximum delay \(\omega_{\max}\).
% and the delay-generation process. 
In the case study described in Section~\ref{numerical:benchmarks}, the values of \(\omega_{\max}\) correspond to a zero-delay case (requests may be realized in the same period before rewards are drawn), a maximum delay of 24 hours (one day), and a maximum delay of 168 hours (one week). We implement a stochastic delay-generation process in which each \(\omega^{(t)}_{i,j}\) is drawn uniformly from \(\{0,\dots,\omega_{\max}\}\).

% The left column of Figure~\ref{fig:sweep_delay} considers a worst-case delay process that selects delays to make the intermediate workforce as poor as possible. Formally, for each \((i,j)\in R_t\), the realized delay is
% \[
% \omega^{(t)}_{i,j}=
% \begin{cases}
%     \omega_{\max}, & \text{if } \mu_i \le \mu_j,\\
%     1, & \text{if } \mu_i > \mu_j.
% \end{cases}
% \]
% Intuitively, this corresponds to a regime in which poor performers are highly available while strong performers are relatively unavailable. The right column considers a stochastic delay process, where each \(\omega^{(t)}_{i,j}\) is drawn uniformly from \(\{0,\dots,\omega_{\max}\}\). Each simulation is run on 20 independently generated instances. 

% The shaded region represents \(\pm\)1 standard deviation of the collected trajectories. All other parameters are the same as in the benchmark experiment from Section~\ref{numerical:benchmarks}. For each delay specification, we simulate the performance of \textsc{AHT}, \textsc{OMM}, and \textsc{Optimistic-Hire}, shown in rows 1, 2, and 3, respectively.

% \begin{figure}
%     \centering
%     \begin{minipage}[t]{0.45\linewidth}
%         \centering
%         \includegraphics[width=\linewidth]{paper-images/AHT_delay_sweep_wc.png}
%     \end{minipage}
%     \hfill
%     \begin{minipage}[t]{0.45\linewidth}
%         \centering
%         \includegraphics[width=\linewidth]{paper-images/AHT_delay_sweep_stoch.png}
%     \end{minipage}

%     \vspace{0.5em}
%     \begin{minipage}[t]{0.45\linewidth}
%         \centering
%         \includegraphics[width=\linewidth]{paper-images/OMM_delay_sweep_wc.png}
%     \end{minipage}
%     \hfill
%     \begin{minipage}[t]{0.45\linewidth}
%         \centering
%         \includegraphics[width=\linewidth]{paper-images/OMM_delay_sweep_stoch.png}
%     \end{minipage}
    
%     \vspace{0.5em}
%     \begin{minipage}[t]{0.45\linewidth}
%         \centering
%         \includegraphics[width=\linewidth]{paper-images/OH_delay_sweep_wc.png}
%     \end{minipage}
%     \hfill
%     \begin{minipage}[t]{0.45\linewidth}
%         \centering
%         \includegraphics[width=\linewidth]{paper-images/OH_delay_sweep_stoch.png}
%     \end{minipage}
%     \caption{Simulated performance of \textsc{AHT}, \textsc{OMM}, and \textsc{Optimistic-Hire} with various choices of \(\omega_{\max}\) and  worst-case (left) and stochastic (right) delay-generation processes.}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.3\linewidth}
        \centering
        \includegraphics[width=\linewidth]{paper-images/benchmark_delay_stochastic_omega_168.png}
    \end{minipage}
...
Figure~\ref{fig:sweep_delay} yields two main observations. First, \textsc{Optimistic-Hire} outperforms \textsc{AHT} even in the zero-delay regime \((\omega_{\max}=0)\). In our implementation, this is the case in which accepted replacements can be realized in the same environment step before period-\(t\) rewards are generated, so no delay-specific feasibility frictions remain beyond the switching-cost structure inherited from \cite{agrawal1990switching}. The performance gap therefore cannot be attributed solely to the way \textsc{AHT} is modified for delayed replacements, and instead suggests that \textsc{Optimistic-Hire} more effectively balances exploration and switching even when requests can be implemented immediately.

Second, the variability of regret for \textsc{AHT} increases substantially with the delay length, whereas the variability of \textsc{Optimistic-Hire} remains comparatively stable. This reflects the deterministic switching schedule used by \textsc{AHT}: when delays are long, realized replacements can lag behind the prescribed schedule, so the active workforce and the amount of information collected at a switching epoch may differ substantially from what the policy anticipates. This points to the adaptive switching rule in \textsc{Optimistic-Hire} as an important source of robustness, since it responds to realized observations rather than calendar time alone. Appendix~\ref{sec:baseline-diagnostics} also shows that delay-adapted versions of \textsc{AHT} and \textsc{OMM} frequently generate infeasible requests under delays, which further helps explain why delay awareness matters empirically.


\subsection{Effects of Switching Cost Magnitude}
In Figure~\ref{fig:sweep_c}, we examine the effect of switching costs on \textsc{AHT}, \textsc{OMM}, and \textsc{Optimistic-Hire} under the same setup as Section~\ref{numerical:benchmarks}, with \(k=150\), \(m=100\), and \(\omega_{\max}=8\). The first plot shows cumulative regret when there are no costs \((c=0)\). In this setting, \textsc{Optimistic-Hire} performs significantly better than \textsc{AHT}. Notably, it also recovers performance competitive with \textsc{OMM}. When costs are larger, with \(c\in \{24, 168\}\), \textsc{Optimistic-Hire} performs substantially better than both other policies. This is consistent with the role of the horizon-aware screening constraint in \ChooseTarget: as switching becomes more expensive, ruling out late or weakly justified replacements becomes more valuable.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.3\linewidth}
        \centering
        \includegraphics[width=\linewidth]{paper-images/benchmark_cost_168.png}
    \end{minipage}
...
Taken together, the numerical results suggest three distinct mechanisms behind the performance of \textsc{Optimistic-Hire}. The no-delay comparison indicates that the optimistic, horizon-aware selection rule is already competitive even before delays matter. The delay sweep then isolates the benefit of the adaptive switching schedule, which appears to stabilize performance when realized completion times vary substantially. Finally, the cost sweep shows why screening replacements through the remaining-horizon constraint matters: when workforce changes are expensive, avoiding marginal late-horizon switches becomes increasingly valuable. Appendix~\ref{sec:oh-ablations} makes this decomposition more explicit. In the benchmark calibration, fixed-calendar switching worsens long-run performance, removing horizon screening has almost no effect at \(c=8\) but does increase regret in a high-cost stress test, and random pairing is nearly indistinguishable from rank matching under i.i.d.\ delays. This last finding is consistent with Propositions~\ref{lem:iid-rank-matching-E} and \ref{lem:iid-rank-matching-Var}: under i.i.d.\ delays, pairing is primarily a transition-quality and variability issue rather than a driver of the leading regret term. In a small adversarial-delay stress test, random pairing performs modestly worse and is noticeably more variable, which is directionally consistent with the conservative surrogate analysis in Appendix~\ref{sec:bijection_analysis}. The additional \(\gamma\)-sensitivity experiments reported in the appendix complement this interpretation by showing that the bound-based choice of \(\gamma\) is empirically reasonable and that the best switching frequency shifts with the gap structure in the direction suggested by the theory.

\section{Concluding Remarks}\label{sec:conclusion}
This paper studies sequential hiring decisions in contingent labor environments. We formulate hiring as a bandit problem with switching costs and worker-availability uncertainty through an action-delay structure. We propose a learning-based hiring policy that balances information acquisition against switching costs and the performance degradation induced by delayed actions. We show that the proposed algorithm attains a regret bound whose leading-order dependence on \(T\) matches the lower bound up to logarithmic and lower-order friction terms. In particular, when switching frequency is controlled appropriately, the policy achieves cumulative regret with the same \(\tilde{O}(\sqrt{kmT})\) dependence as the lower bound for combinatorial delayed and costly replacement. When switching costs are large and grow with the time horizon \(T\), the algorithm parameter can be chosen to minimize total regret. Our numerical experiments are supportive rather than exhaustive: across the calibrated settings and delayed-action benchmark adaptations we study, the proposed policy compares favorably with both classical switching-cost algorithms designed for instantaneous actions and several operationally motivated heuristics. The additional ablations indicate that the adaptive switching and horizon-aware selection components drive most of those gains in our benchmark calibration, while the pairing rule is best viewed as a complementary refinement for managing intermediate teams under less favorable delay realizations.
...
The code used in this paper is available at \url{https://github.com/leechr001/hiring-bandit}. The repository includes the simulation environment, implementations of all policies reported in the paper, and scripts to reproduce the figures, tables, internal ablations, and additional diagnostics reported in the appendices. 
...
\begin{eqnarray*}
    ...
    &&+\;(\omega_{\max}+c) m\sqrt{\frac{kT}{\gamma}} + (\omega_{\max}+c)mk.
\end{eqnarray*}
...
Finally, since each epoch can change up to \(m\) arms, the switching regret is at most \(cm\) times the number of epochs completed by time \(T\). Solving \(d_{\ell+1} \leq T\) yields \(\ell \leq \sqrt{kT/\gamma} + k\). Therefore, we obtain the switching-cost bound stated in Lemma~\ref{lem:sc-bound-final},
\[
    J_T \leq cm\left(\sqrt{\frac{kT}{\gamma}} + k\right).
\]

This bound reinforces a central message from the switching-cost literature: limiting the frequency of action changes is essential for favorable regret scaling. The key distinction in our setting is that adjustment timing is not imposed exogenously; instead, it is governed by an adaptive, data-dependent threshold rule that depends endogenously on both learning progress and execution delays. This endogenous timing structure creates challenges not present in settings with fixed or externally specified switching schedules and therefore requires a new analytical approach.
...
To complete the argument, we bound the regret incurred during the transition phases. During these phases, the arbitrary nature of replacement times obscures the identity of the best omitted optimal arm, so we do not attempt a refined bound on \(\Delta_{i_t^*,j}\). Moreover, suboptimal arms may persist in transition phases even after they can no longer appear in a block phase, for example if their replacements are delayed. Instead, we use a worst-case bound. Each epoch contains at most \(\omega_{\max}\) transition periods, and each such period incurs regret at most \(m\Delta_{1,k} \leq m\), since all gaps lie in \([0,1]\). It therefore suffices to bound the number of epochs completed within \(T\) rounds. The epoch-count bound from the switching-cost analysis yields at most \(\sqrt{kT/\gamma}+k\) epochs. Putting these pieces together gives the statement of Lemma~\ref{lem:transition-regret},
\[
    (\text{Transition regret})
    \le
    \omega_{\max} m\!\left(\sqrt{\frac{kT}{\gamma}}+k\right).
\]
Combining this with the contribution from the block phases completes the sampling-regret bound. Combining the result further with the switching-regret bound completes the proof.
...
\noindent {\bf Proof.}
    From the definition of \TimeToSwitch\,\,(Algorithm \ref{alg:TimeToSwitch}) we have that the length of an epoch is either the block threshold \(N(\ell)\) or the longest delay, whichever comes last. Thus,
...
\subsection{Final bound on switching regret}
Finally, we prove a bound on the switching regret.
...
\begin{lemma}[Gap free switching regret]\label{lem:sc-bound-final}
    The switching regret satisfies
    \[
        J_T \leq cm\sqrt{\frac{kT}{\gamma}}+cmk.
    \]
\end{lemma}
\noindent {\bf Proof.}
    By construction, the number of switches by time \(T\) is at most \(cm\) times the number of epochs that can be completed within \(T\) rounds. By Lemma \ref{lem:max_epochs},
    \[J_T \leq cm\cdot \left(\sqrt{\frac{kT}{\gamma }}+k\right)=cm\sqrt{\frac{kT}{\gamma}}+cmk\]
\Halmos  

We can also derive an instance-dependent switching bound using Lemma~\ref{lem:ell-t-bound} together with Corollary \ref{cor:final-phase} (proved in the next section).
...
Similarly, we can derive an instance-dependent switching regret.
\begin{lemma}[Instance-dependent switching regret]\label{lem:delta-sc-bound-final}
...
\end{lemma}
\noindent {\bf Proof.}
    By construction, the number of switches by time \(T\) is at most \(cm\) times the number of epochs that can be completed within \(T\) rounds. By Lemma \ref{lem:delta-max-epoch},
...
\begin{lemma}\label{lem:ordered-intervals}
    ...
    \[\ell_{0,j}:=1\leq \ell_{1,j}\leq \ell_{2,j}\leq \cdots \leq \ell_{m,j}\]
\end{lemma}
\noindent {\bf Proof.}
...
\noindent {\bf Proof.}
    ...
    \begin{equation}\label{eq:block-set}
        \{t \in \mathcal{B}_\ell : \ell_{i-1,j}+1 \leq \ell \leq \ell_{i,j}\}.
    \end{equation}
    Since \(A_t = U_\ell\) for all \(t \in \mathcal{B}_\ell\), we can rewrite
    \begin{align*}
        \sum_{i=1}^m\bE{\sum_{\ell \in [\ell_{i-1,j}+1, \ell_{i,j}+1)}\sum_{t \in \mathcal{B}_\ell}\Delta_{i^*_t,j}\ind{j \in U_\ell, \mathcal{G}_j(d_\ell)}}
        &= \sum_{i=1}^m\Delta_{i,j}\bE{\sum_{t \in (\ref{eq:block-set})}\ind{j \in A_t, \mathcal{G}_j(d_{\epoch(t)})}}\\
        &=\sum_{i=1}^m\Delta_{i,j}\bE{|(\ref{eq:block-set})|\cdot \ind{j\in A_t, \mathcal{G}_j(d_{\epoch(t)})}}
    \end{align*}
    Thus, it suffices to bound the contribution from the set \((\ref{eq:block-set})\). Observe that
    \[
        (\ref{eq:block-set}) = \{t : d_{\ell_{i-1,j}+1} \leq t < d_{\ell_{i,j}+1}, t \in \mathcal{B}_{\epoch(t)}, j\in A_t\}.
    \] 
    Intuitively, the set \((\ref{eq:block-set})\) consists of the distinguishability interval shifted to the right by some ``extra pulls'' determined by the maximum block length (see Figure \ref{fig:elimination-timeline}). This motivates us to define 
    ...
    \[
        |(\ref{eq:block-set})| = |\{t\in \mathcal{B}_{\epoch(t)} : t_{i-1,j} \leq t < t_{i,j}, j\in A_t\}| + |\tilde{B}_{i,j}| -  |\tilde{B}_{i-1,j}|
    \]
    
    Therefore,
    \begin{align*}
        &\sum_{i=1}^m\bE{\sum_{t \in (\ref{eq:block-set})}\ind{j \in A_t, \mathcal{G}_j(d_{\epoch(t)})}}\\
...
We control the regret from the transition phase by limiting the number of epochs, and thus the number of transition periods. Since the expected number of periods in the transition phase is bounded by \(\omega_{\max}\), and the regret in each period is at most \(m\Delta_{1,k}\leq m\), the same techniques used to bound the switching costs can be used to control the sampling regret from transition phases. 

\begin{lemma}[Gap-free regret from transitions]\label{lem:transition-regret}
    \[\bE{\sum_{t=1}^T\Delta_{i^*_t, j}\ind{j\in A_t, t\in \mathcal{T}_{\epoch(t)}}}\leq \omega_{\max}m\sqrt{\frac{kT}{\gamma}}+\omega_{\max}mk.\]
\end{lemma}
\noindent {\bf Proof.}
    In each epoch, the expected length of each delay is upper bounded by \(\omega_{\max}\). Therefore the expected regret incurred in each transition phase is at most \(\omega_{\max} m\) since \(\Delta_{i,j}\in [0,1]\) for all \(i,j\) and \(\omega_{i,j}^{(t)}\) is independent. From Lemma \ref{lem:max_epochs} we have that the regret from transitions are bounded by
    \[\omega_{\max}mL\leq \omega_{\max}m\left(\sqrt{\frac{kT}{\gamma}}+k\right)\]
...
\begin{lemma}[Instance-dependent regret from transitions]\label{lem:delta-transition-regret}
    ...
    \[\bE{\sum_{t=1}^T\Delta_{i^*_t, j}\ind{j\in A_t, t\in \mathcal{T}_{\epoch(t)}}}\leq 2\omega_{\max}mk\left(\left(\sum_{j>m}\frac{2}{\Delta_{m,j}}\right)\sqrt{\frac{\log T}{k\gamma}}+1+\frac{1}{k}+\frac{k}{\gamma T}+\frac{k^2}{T^2}\right)\]
    and when \(T>k\),
    \[\bE{\sum_{t=1}^T\Delta_{i^*_t, j}\ind{j\in A_t, t\in \mathcal{T}_{\epoch(t)}}} \in O\!\left(\left(\sum_{j>m}\frac{1}{\Delta_{m,j}}\right)\cdot \omega_{\max}m\sqrt{\frac{k\log T}{\gamma}}\right)\]
\end{lemma}
\noindent {\bf Proof.}
    In each epoch, the transition phase can incur regret of at most \(\omega_{\max} m\), since \(\Delta_{i,j}\in [0,1]\) for all \(i,j\). From Lemma \ref{lem:delta-max-epoch}, when \(\Delta>0\) we have that the regret from transitions is bounded by
    \[\bE{\omega_{\max}mL}\leq 2\omega_{\max}mk\left(\left(\sum_{j>m}\frac{2}{\Delta_{m,j}}\right)\sqrt{\frac{\log T}{k\gamma}}+1+\frac{1}{k}+\frac{k}{\gamma T}+\frac{k^2}{T^2}\right)\]
...
\begin{lemma}\label{lem:instance-independent-regret}
    ...
    \begin{align*}
        ...
        &\quad + ~\omega_{\max} m\sqrt{\frac{kT}{\gamma}} + \omega_{\max}mk
...
\begin{theorem}
    ...
    \begin{align*}
        \Regret(T)&\leq \left(\sum_{j>m}\frac{4}{\Delta_{m,j}}\right) \left( 2\log T+(\omega_{\max}+c)m\sqrt{\frac{k\log T}{\gamma }}\right)\\
        ...
        &\quad +~2(\omega_{\max}+c)mk\left(1+\frac{1}{k}+\frac{k}{\gamma T}+\frac{k^2}{T^2}\right) + (c+1)(k-m)
    \end{align*}
    In particular
    \begin{align*}
        \limsup_{T\to\infty}\frac{\Regret(T)}{\log T}&\leq \sum_{j>m} \frac{8}{\Delta_{m,j}}
...
\noindent{\bf Proof.}
    ...
    \begin{align*}
        ...
        &\quad + ~ 2\omega_{\max}mk\left(\left(\sum_{j>m}\frac{2}{\Delta_{m,j}}\right)\sqrt{\frac{\log T}{k\gamma}}+1+\frac{1}{k}+\frac{k}{\gamma T}+\frac{k^2}{T^2}\right)\\
...
\begin{align*}
    Z(\omega, \pi):=
    \inf_{(\boldsymbol{\mu}^-,\boldsymbol{\mu}^+)\in\Theta}\sum_{i\in A^-}\left(\omega_{i,\pi(i)}\,\mu_i+(\omega_{\max}-\omega_{i,\pi(i)})\,\mu_{\pi(i)}\right)
\end{align*}
...
\[Z(\pi;\omega )=
    ~\sum_{i\in A^-}\left(\omega_{i,\pi(i)}\,\LCB_i+(\omega_{\max}-\omega_{i,\pi(i)})\,\LCB_{\pi(i)}\right),\]
...
We first consider a conservative criterion in which execution delays are allowed to vary arbitrarily, subject only to the known upper bound \(\omega_{\max}\). This corresponds to evaluating each bijection under the least favorable delay realization consistent with the model.
...
where \(\omega^\pi\) denotes the delays \(\{\omega_{i,\pi(i)}\}\) induced by the pairing rule \(\pi\). The infimum ranges over all choices \(\omega_{i,\pi(i)} \in \{0,\dots,\omega_{\max}\}\). In this setting the rank-matching bijection minimizes the regret incurred by intermediate workforces.
...
\noindent {\bf Proof of Proposition \ref{lem:wc-rank-matching}.}
    Fix an arbitrary bijection \(\pi\). For each \(i\in A^-\), the term involving \(\omega_{i,\pi(i)}\) is linear, and the infimum over \(\omega_{i,\pi(i)}\in\{0,\dots,\omega_{\max}\}\) is attained at an endpoint:
    \begin{eqnarray*}
    \inf_{\omega_{i,\pi(i)}}Z(\pi;\omega )=
       \omega_{\max}\,\LCB_{\pi(i)} + \omega_{\max}\,(\LCB_i-\LCB_{\pi(i)})^+.
    \end{eqnarray*}
    Therefore, 
   \begin{eqnarray*}
        \Phi^{\mathrm{WC}}(\pi)= \left(\sum_{i\in A^-}\omega_{\max}\LCB_{\pi(i)}\right)+ \left(\sum_{i\in A^-}\omega_{\max}\,(\LCB_i-\LCB_{\pi(i)})^+\right)
    \end{eqnarray*}
    Since
    \[\sum_{i\in A^-}\omega_{\max}\LCB_{\pi(i)}=\sum_{j\in A^+}\omega_{\max}\LCB_j\]
...
\noindent {\bf Proof of Proposition \ref{lem:iid-rank-matching-E}}. 
    ...
    \[\bE{\sum_{i\in A^-}\omega_{i,\pi(i)}\LCB_i + (\omega_{\max}-\omega_{i,\pi(i)})\LCB_{\pi(i)}}\]
    Using linearity of expectation and using \(\bE{\omega_{i,\pi(i)}}=\mu\) for all \(i\in A^-\),
    \[\mu\sum_{i\in A^-}\LCB_i + (\omega_{\max}-\mu)\sum_{i\in A^+}\LCB_{\pi(i)}\]
    Then because 
...
\noindent {\bf Proof of Proposition \ref{lem:iid-rank-matching-Var}}. 
    ...
    \[\omega_{\max}\sum_{j\in A^+}\LCB_{j} +\sum_{i\in A^-}\omega_{i,\pi(i)}\bigl(\LCB_i-\LCB_{\pi(i)}\bigr)\]
...
\begin{table}[H]
    ...
    \midrule
    1 month & $3{,}321 \pm 197$ & $2{,}531 \pm 310$ & $8{,}553 \pm 223$ \\
    12 months & $7{,}064 \pm 388$ & $12{,}970 \pm 2{,}818$ & $41{,}079 \pm 820$ \\
    3 years & $10{,}057 \pm 332$ & $25{,}254 \pm 3{,}584$ & $74{,}024 \pm 1{,}714$ \\
    5 years & $11{,}616 \pm 317$ & $39{,}240 \pm 3{,}410$ & $95{,}838 \pm 2{,}211$ \\
    \bottomrule
    \end{tabular*}
    ...
    Horizon & Optimistic-Hire & AHT & OMM \\
    \midrule
    1 month & $8.46\% \pm 0.50\%$ & $6.45\% \pm 0.79\%$ & $21.78\% \pm 0.57\%$ \\
    12 months & $1.48\% \pm 0.08\%$ & $2.71\% \pm 0.59\%$ & $8.60\% \pm 0.17\%$ \\
    3 years & $0.70\% \pm 0.02\%$ & $1.76\% \pm 0.25\%$ & $5.16\% \pm 0.12\%$ \\
    5 years & $0.49\% \pm 0.01\%$ & $1.64\% \pm 0.14\%$ & $4.01\% \pm 0.09\%$ \\
    \bottomrule
    \end{tabular*}
    ...
    Horizon & Optimistic-Hire & AHT & OMM \\
    \midrule
    1 month & $5{,}313 \pm 14$ & $3{,}315 \pm 369$ & $11{,}207 \pm 209$ \\
    12 months & $10{,}570 \pm 393$ & $15{,}015 \pm 1{,}311$ & $43{,}114 \pm 639$ \\
    3 years & $14{,}259 \pm 379$ & $32{,}664 \pm 1{,}434$ & $70{,}796 \pm 1{,}214$ \\
    5 years & $16{,}155 \pm 409$ & $49{,}393 \pm 1{,}917$ & $88{,}270 \pm 1{,}288$ \\
    \bottomrule
    \end{tabular*}
    ...
    Horizon & Optimistic-Hire & AHT & OMM \\
    \midrule
    1 month & $23.68\% \pm 0.06\%$ & $14.77\% \pm 1.64\%$ & $49.94\% \pm 0.93\%$ \\
    12 months & $3.87\% \pm 0.14\%$ & $5.50\% \pm 0.48\%$ & $15.79\% \pm 0.23\%$ \\
    3 years & $1.74\% \pm 0.05\%$ & $3.99\% \pm 0.18\%$ & $8.64\% \pm 0.15\%$ \\
    5 years & $1.18\% \pm 0.03\%$ & $3.62\% \pm 0.14\%$ & $6.47\% \pm 0.09\%$ \\
    \bottomrule
    \end{tabular*}
...
Next, we report additional diagnostics for the delayed-action baseline adaptations discussed in Section~\ref{numerical:benchmarks}.

\subsection{Diagnostics for Delayed Baseline Adaptations}\label{sec:baseline-diagnostics}
The main benchmark uses random pairing for the delayed-action adaptations of \textsc{AHT} and \textsc{OMM}. To better understand how much of the benchmark gap is driven by that choice, and how often the adapted baselines request infeasible replacements, we ran a small additional diagnostic experiment under the same calibration as Section~\ref{numerical:benchmarks}. Table~\ref{tab:benchmark-adaptation-diagnostics} reports five-year regret, the share of requested replacement pairs that were discarded because they conflicted with pending replacements, the total number of accepted replacements, the number of switch periods, and the share of periods with pending replacements.

\begin{table}[t]
    \centering
    \small
    \setlength{\tabcolsep}{4pt}
    \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}} l l r r r r r}
    \toprule
    Policy & Pairing & 5-year regret & Rejected share & Accepted repl. & Switch periods & Pending-period share \\
    \midrule
    AHT & Random & $40{,}405$ & $60.48\%$ & $2{,}867.5$ & $2{,}792.5$ & $50.53\%$ \\
    AHT & Rank-matching & $40{,}738$ & $60.49\%$ & $2{,}871.5$ & $2{,}799.5$ & $50.84\%$ \\
    OMM & Random & $96{,}292$ & $88.76\%$ & $11{,}049.5$ & $8{,}818.5$ & $87.73\%$ \\
    OMM & Rank-matching & $94{,}700$ & $89.00\%$ & $10{,}833.5$ & $8{,}177.0$ & $87.78\%$ \\
    \bottomrule
    \end{tabular*}
    \caption{Illustrative diagnostic statistics for the delayed-action benchmark adaptations under the calibration of Section~\ref{numerical:benchmarks}. Each row averages two additional replications. Rejected share reports the fraction of requested replacement pairs that were dropped because they conflicted with in-progress replacements.}
    \label{tab:benchmark-adaptation-diagnostics}
\end{table}

The qualitative message is twofold. First, changing the pairing rule does affect the delayed-action baseline adaptations, but it does not overturn the overall ranking observed in the main benchmark. Second, discarded requests are far from negligible, particularly for \textsc{OMM}, which confirms that feasibility frictions materially affect off-the-shelf policies once delayed actions are introduced.

\subsection{Ablations of \textsc{Optimistic-Hire}}\label{sec:oh-ablations}
Because the external benchmarks are adapted policies, it is also useful to isolate the contribution of the design choices inside \textsc{Optimistic-Hire} itself. Table~\ref{tab:oh-ablations-main} reports three ablations under the benchmark calibration of Section~\ref{numerical:benchmarks}: fixed calendar-time switching in place of the adaptive count-based rule, removal of the horizon-aware screen in \ChooseTarget, and random pairing in place of rank matching.
...
\begin{table}[t]
    ...
    \midrule
    \textsc{Optimistic-Hire} & $7{,}088 \pm 275$ & $11{,}289 \pm 285$ \\
    OH + fixed calendar switching & $7{,}049 \pm 251$ & $11{,}615 \pm 201$ \\
    OH without horizon screening & $7{,}088 \pm 275$ & $11{,}299 \pm 293$ \\
    OH + random pairing & $7{,}088 \pm 275$ & $11{,}289 \pm 285$ \\
    \bottomrule
    \end{tabular*}
    \caption{Ablations of \textsc{Optimistic-Hire} under the benchmark calibration of Section~\ref{numerical:benchmarks}. Each row averages four additional replications.}
    \label{tab:oh-ablations-main}
\end{table}

The benchmark ablation yields a useful hierarchy. The adaptive switching rule has the clearest effect in this calibration: replacing it by a fixed calendar schedule has little short-run impact but worsens five-year regret. By contrast, removing the horizon screen or randomizing the pairing rule is nearly inconsequential in the benchmark setting. This is informative rather than contradictory. The benchmark cost \(c=8\) is modest, so the screening rule is only weakly binding, and the delays are i.i.d., so Propositions~\ref{lem:iid-rank-matching-E} and \ref{lem:iid-rank-matching-Var} suggest that pairing should mainly affect transition variability rather than the leading mean performance.

To make these latter mechanisms more visible, Table~\ref{tab:oh-ablations-stress} reports two small stress tests. In the first, switching costs are increased to \(c=168\), which makes the horizon screen more consequential. In the second, delays are chosen adversarially so that beneficial replacements complete slowly and detrimental ones complete quickly, making the pairing decision more exposed. These experiments are illustrative rather than exhaustive, but they help explain which components of \textsc{Optimistic-Hire} matter most in which regimes.
...
\begin{table}[t]
    ...
    \midrule
    High switching cost (\(c=168\)) & \textsc{Optimistic-Hire} & $38{,}460 \pm 1{,}000$ \\
    High switching cost (\(c=168\)) & OH without horizon screening & $38{,}891 \pm 1{,}136$ \\
    Adversarial delays & \textsc{Optimistic-Hire} & $11{,}609 \pm 138$ \\
    Adversarial delays & OH + random pairing & $11{,}710 \pm 239$ \\
    \bottomrule
    \end{tabular*}
    \caption{Stress-test ablations for \textsc{Optimistic-Hire}. Each row averages four additional replications.}
    \label{tab:oh-ablations-stress}
\end{table}

The high-cost stress test shows that the horizon-aware screen matters once late switches become expensive. The adversarial-delay stress test shows a more modest mean effect from pairing, but also a noticeable increase in variability under random pairing. This reinforces the intended positioning of the pairing contribution in the paper: it is a useful implementation refinement for controlling intermediate-team behavior, especially under unfavorable delay realizations, but the main regret improvements documented in the benchmark calibration come primarily from the adaptive switching and horizon-aware selection architecture.
