...
We develop a learning-based hiring policy, \textsc{Optimistic-Hire}, that makes hiring decisions sequentially through learning cycles. We show that the regret of the proposed policy matches the leading-order lower bound in its dependence on the time horizon. 
...
\]
with ties broken arbitrarily, but in a consistent manner. This ordering is unknown to the firm, which must learn which workers perform best through repeated assignments and observation. At the beginning of each period \(t = 1, \dots, T\), the firm must staff \(m < k\) identical positions. Let \(A_t\) be the set of workers being employed in period \(t\). We refer to \(A_t\) as the \emph{active workforce} or \emph{active set}. The assumption that the firm is filling \(m\) identical positions is natural in settings where workforce size is constrained by equipment or logistical capacity, and where any pre-screened worker is preferable to leaving a position unfilled.
...
When the firm wishes to change work assignments by replacing worker \(i\) with worker \(j\), the change does not take effect immediately. Instead, the action enters a delay process, capturing frictions such as onboarding steps, platform-mediated communication, scheduling constraints, or the worker's own availability, before worker \(j\) becomes active. We model this friction by associating each replacement \((i,j)\) initiated in period \(t\) with an independent, positive-integer-valued delay \(\omega_{i,j}^{(t)}\). We assume that \(\bE{\omega_{i,j}^{(t)}} \le \omega_{\max} < \infty\) for all \(i,j\) and \(t\), where \(\omega_{\max}\) is known to the firm. Since decisions are made at the beginning of each period, the event \(\omega_{i,j}^{(t)}=1\) corresponds to the case in which the replacement takes effect without delay. The analysis can also be extended to adaptive or adversarial delay processes when the delays have finite support.
...
To model these frictions explicitly, we assume that each period proceeds in three stages. First, the active workforce \(A_t\) %, already determined by replacements completed in earlier periods, 
generates observable performance outcomes. Second, based on these observations, the firm may initiate a set of replacement decisions \(R_t\) and incur the associated switching costs (introduced below). 
Third, some pending replacements complete and the active workforce updates. Hence the firm does not directly control the current workforce in period \(t\); rather, it controls the replacement decisions \(R_t\), which shape future workforces through their delayed realization.
...
\[
    \mathcal{E}_t := \{(i,j) \;:\; C_{i,j}^{(n)} = t \text{ for some } n < t\}
\]
...
The active roster evolves only through the completion of previously initiated replacements. For any replacement \((i,j)\) with completion time \(C_{i,j}^{(\ell)} = t\), worker \(i\) leaves the roster and worker \(j\) enters it in period \(t\). The workforce update is therefore
\[
    A_t
...
\begin{algorithm}
    ...
    \For{time \(t = 1,2,\dots,T\)}
        \State Observe current active set \(A_t\) and performance \(X_t^{(i)}\) for each \(i\in A_t\)
        \State Update statistics \(\mathbf{n}_t\) and \(\boldsymbol{\mu}_t\)
        ...
        \EndIf
        \State The environment realizes a set of completed replacements \(\mathcal{E}_t\)
        \State Update active workforce: \( A_{t+1}\gets\big( A_{t} \setminus \{  i : (i,j) \in \mathcal{E}_t  \} \big) \cup \{  j : (i,j) \in \mathcal{E}_t  \} \)
    \EndFor
    \end{algorithmic}
\end{algorithm}

Algorithm~\ref{alg:Optimistic-Hire} reduces the policy design problem to three interacting components. The switching rule determines when to switch target workforces and incur replacement costs, the selection rule identifies the target workforce given current information, and the pairing rule translates this target into a concrete set of replacements. We next define each component in turn, beginning with the switching rule, followed by the pairing rule, and finally the selection rule.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Require Current empirical mean vector \(\boldsymbol{\mu}_t=(\bar{\mu}_1(t),\dots,\bar{\mu}_k(t))\)
        \Statex \textit{//===========================}
        \State \Return Target workforce \(U\) which (approximately) solves the optimization problem:
...
\begin{remark}[Computation of \textsc{ChooseTarget}]
    With the added constraint, it is no longer obvious that the solution \(U\) can be computed efficiently, even approximately. In Appendix \ref{sec:comp_of_selection_rule}, we develop a \((1-\epsilon)\)-approximation algorithm that relies on a monotonicity property proved in Appendix \ref{sec:sampling_proof}.
\end{remark}
...
\label{numerical}
This section presents numerical experiments designed to illustrate the empirical behavior of the proposed hiring policy. We begin by benchmarking empirical performance in a fulfillment-center-inspired setting. We compare \textsc{Optimistic-Hire} with bandit algorithms from the literature and with heuristics motivated by operational practice.
...
We run the simulation on \(25\) independently generated instances, with worker means drawn uniformly from \([0.3,0.7]\). Figure~\ref{fig:benchmark} plots the mean trajectory for each policy, with shaded bands showing \(\pm 1\) standard deviation across runs.
...
The main message is that \textsc{Optimistic-Hire} provides the strongest overall performance across the full planning horizon. Although some benchmark policies perform well over particular portions of the horizon, \textsc{Optimistic-Hire} is the only method that remains consistently near the frontier throughout and is best at the terminal five-year horizon. In particular, at five years it achieves the smallest cumulative regret and normalized loss among all policies tested.
...
At five years, \textsc{Optimistic-Hire} reduces cumulative regret by about \(69\%\) relative to \textsc{AHT}, about \(88\%\) relative to \textsc{OMM}, and about \(48\%\) relative to \textsc{WorkTrial}. It is also the most stable policy across instances. Additionally, \textsc{Optimistic-Hire} reduces the variability of cumulative regret by about \(78\%\) relative to \textsc{AHT}, about \(79\%\) relative to \textsc{WorkTrial}, and about \(82\%\) relative to \textsc{OMM}. Thus, \textsc{Optimistic-Hire} not only achieves the best mean performance at the target horizon, but does so substantially more reliably across heterogeneous instances. In Appendix \ref{sec:benchmark_tables} we provide tables listing the cumulative regret and normalized loss evaluated at horizons 1 month, 12 months, 3 years, and 5 years.

Figure~\ref{fig:benchmark_50_150} reports results for the lower-utilization regime with \(m=50\), so that a smaller fraction of the worker pool is employed in each period. This provides a robustness check with respect to the staffing ratio. The qualitative pattern is unchanged: \textsc{Optimistic-Hire} again delivers the strongest overall performance, remaining competitive early and achieving the best outcomes over the 3-year and 5-year horizons. While \textsc{TunedThreshold} performs best in the short run, it relies on tuning parameters that depend on unknown problem characteristics. Accordingly, it should be viewed as a best-case benchmark rather than a deployable policy. Appendix~\ref{sec:threshold-robustness} shows that its performance can deteriorate substantially under even mild perturbations of the underlying unknown problem parameters.
...
In the case study described in Section~\ref{numerical:benchmarks}, the values of \(\omega_{\max}\) correspond to maximum delays of 1 hour (replacements complete in the period after they are requested), 24 hours (one day), and 168 hours (one week). We implement a stochastic delay-generation process in which each \(\omega^{(t)}_{i,j}\) is drawn uniformly from \(\{1,\dots,\omega_{\max}\}\).
...
% \end{cases}
% \]
% Intuitively, this corresponds to a regime in which poor performers are highly available while strong performers are relatively unavailable. The right column considers a stochastic delay process, where each \(\omega^{(t)}_{i,j}\) is drawn uniformly from \(\{1,\dots,\omega_{\max}\}\). Each simulation is run on 20 independently generated instances. 
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.3\linewidth}
        \centering
        \includegraphics[width=\linewidth]{paper-images/benchmark_delay_stochastic_omega_1.png}
    \end{minipage}
    \hfill
    \begin{minipage}{0.3\linewidth}
        \centering
...
Figure~\ref{fig:sweep_delay} yields two main observations. First, \textsc{Optimistic-Hire} outperforms \textsc{AHT} even in the instantaneous-replacement regime \((\omega_{\max}=1)\). This case coincides with the setting of \cite{agrawal1990switching}, so no delay-specific adaptations are needed. The performance gap therefore cannot be attributed to the way \textsc{AHT} is modified for delayed replacements, and instead suggests that \textsc{Optimistic-Hire} more effectively balances exploration and switching even when actions are effectively instantaneous. 

Second, the variability of regret for \textsc{AHT} increases substantially with the delay length, whereas the variability of \textsc{Optimistic-Hire} remains comparatively stable. This reflects the deterministic switching schedule used by \textsc{AHT}: when delays are long, realized replacements can lag behind the prescribed schedule, so the active workforce and the amount of information collected at a switching epoch may differ substantially from what the policy anticipates. This highlights the value of the adaptive switching rule in \textsc{Optimistic-Hire}, which responds to realized observations rather than calendar time alone.
...
The code used to generate the simulations in this paper is available at \url{https://github.com/leechr001/hiring-bandit}. 
...
where \(\omega^\pi\) denotes the delays \(\{\omega_{i,\pi(i)}\}\) induced by the pairing rule \(\pi\). The infimum ranges over all choices \(\omega_{i,\pi(i)} \in \{1,\dots,\omega_{\max}\}\). In this setting the rank-matching bijection minimizes the regret incurred by intermediate workforces.
...
For each potential replacement \((i,j)\in A^-\times A^+\), let \(\omega_{i,j}\in\{1,\dots,\omega_{\max}\}\) denote a random delay. We assume that the collection \(\{\omega_{i,j}\}_{(i,j)\in A^-\times A^+}\) is independent and identically distributed according to a common distribution \(P\) supported on \(\{1,\dots,\omega_{\max}\}\). Since the support is bounded, all moments of \(P\) are finite. In particular, we denote
...
\noindent {\bf Proof of Proposition \ref{lem:wc-rank-matching}.}
    Fix an arbitrary bijection \(\pi\). For each \(i\in A^-\), the term involving \(\omega_{i,\pi(i)}\) is linear, and the infimum over \(\omega_{i,\pi(i)}\in\{1,\dots,\omega_{\max}\}\) is attained at an endpoint:
    \begin{eqnarray*}
    \inf_{\omega_{i,\pi(i)}}Z(\pi;\omega )=
       \omega_{\max}\,\LCB_{\pi(i)} + (\omega_{\max}-1)\,(\LCB_i-\LCB_{\pi(i)})^+.
    \end{eqnarray*}
    Therefore, 
   \begin{eqnarray*}
        \Phi^{\mathrm{WC}}(\pi)= \left(\sum_{i\in A^-}\omega_{\max}\LCB_{\pi(i)}\right)+ \left(\sum_{i\in A^-}(\omega_{\max}-1)\,(\LCB_i-\LCB_{\pi(i)})^+\right)
    \end{eqnarray*}
    Since
...
\begin{figure}[h]
    ...
    \caption{Simulated performance of \textsc{Optimistic-Hire} with different choices of \(\gamma\) for 1-year and 5-year horizons.}
...
\begin{figure}[h]
    ...
    \caption{Simulated performance of Optimistic-Hire with various choices of replacement frequency for a small gap instance and a large gap instance.}
    \label{fig:sweep_gamma}
\end{figure}

%\vspace{-.1in}

In Figure \ref{fig:sweep_gamma}, optimality gaps are large (means drawn uniformly in \([0.3,0.8]\)) in the left plot and small (means drawn uniformly in \([0.55, 0.6]\)) in the right plot. In the large gap case, benefits of rapid early exploration are more pronounced. Suboptimal workers can be identified and eliminated relatively quickly, and delaying adjustments leads to sustained employment of clearly inferior configurations. In this setting, a higher switching frequency is advantageous, as it allows the policy to react more quickly to informative performance signals. The plots confirm this behavior: for large-gap instances, smaller values of \(\gamma\) outperform the larger values.
