...
def _build_latex_table(
    *,
    ...
    first_group = labels[:3]
    second_group = labels[3:6]
    ...
    lines.append(r"    ")
    for line in build_subtable(first_group, _format_cumulative, results):
        lines.append(f"    {line}")
    lines.append(r"    ")
    lines.append(r"    \vspace{0.5em}")
    lines.append(r"    ")
    for line in build_subtable(second_group, _format_cumulative, results):
        lines.append(f"    {line}")
    lines.append(r"    ")
    lines.append(r"    \caption{Cumulative regret across policies and planning horizons.}")
    lines.append(r"    \label{tab:cumulative-regret}")
    ...
    lines.append(r"    \setlength{\tabcolsep}{4pt}")
    lines.append(r"    ")
    for line in build_subtable(first_group, _format_normalized, normalized_results):
        lines.append(f"    {line}")
    lines.append(r"    ")
    lines.append(r"    \vspace{0.5em}")
    lines.append(r"    ")
    for line in build_subtable(second_group, _format_normalized, normalized_results):
        lines.append(f"    {line}")
    lines.append(r"    ")
    lines.append(r"    \caption{Normalized loss across policies and planning horizons.}")
    lines.append(r"    \label{tab:loss-percent}")
    lines.append(r"\end{table}")
    lines.append("")
